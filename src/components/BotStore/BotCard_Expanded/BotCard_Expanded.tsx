...
import './BotCard_Expanded.scss';
import { BotCard_ExpandedProps } from './IBotCard_Expanded';
import Graph2 from '../../DashBoard/Graph2/Graph2';
import Rating from '../BS_Rating';
import DetailsCard from '../DetailsCard';

export default class BotCard_Expanded extends PureComponent<BotCard_ExpandedProps> {
  getButton() {
    if (this.props.Type.toLowerCase() == 'chat bot') {
      ...
      return (
        <button className="chatButton uk-button uk-flex uk-flex-1  disabled">
          <div className="uk-flex uk-flex-center uk-flex-middle">RPA</div>
        </button>
      );
    }
  }
  ...
  render() {
    return (
      <div className="wrapper_botcard_expanded uk-flex">
        <div className="botCard_Section uk-overflow-hidden">
          <div className="liveContainer uk-flex uk-flex-1 uk-flex-right ">
            <div
              id="ribbon"
              className={'uk-flex ' + (this.props.Status.toLowerCase().trim() == 'live' ? '' : ' uk-invisible')}>
              <span id="content">{this.props.Status.toLowerCase().trim() == 'live' ? 'LIVE' : ''}</span>
            </div>
          </div>
          ...
          <div className="contetWrapper">
            <div className="title uk-text-truncate uk-">{this.props.Title}</div>
            ...
            <div className="moreInfo uk-flex uk-flex-column">
              <div className="roundEdge uk-text-center"> 1</div>
              <div className="squareEdge uk-flex uk-flex-center uk-flex-middle">
                <div className="SBG badge">/ {this.props.SBG}</div>
                <div className="Function badge">/ {this.props.Function}</div>
              </div>
            </div>
          </div>
        </div>
        <div
          className="uk-flex extraCardSection"
          uk-scrollspy="target: > div; cls: uk-animation-slide-left-medium ; delay: 300">
          <div className="uk-flex extraCard uk-width-1-2">
            <DetailsCard
          ...
          <div className="uk-flex extraCard uk-width-1-2">
            <Graph2
              benefits={this.props.BenefitsCurrentYear}
              viewsCount={this.props.Views}
              BenefitsStartsOn={this.props.BenefitsStartsOn}
            />
          </div>
        </div>
